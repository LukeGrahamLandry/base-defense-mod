...
public class ModRegistry {
    ...
    public static final Supplier<Block> LOOTED_GENERATOR_BLOCK = BLOCKS.register("looted_generator",
            () -> new MaterialGeneratorBlock(false, Block.Properties.of().strength(50.0F, 1200.0F).noOcclusion()));
    ...
    public static final Supplier<Block> BASE_BLOCK = BLOCKS.register("base_block",
            () -> new BaseBlock(Block.Properties.of().strength(5.0F, 1200.0F).noOcclusion()));

    public static final Supplier<Block> TURRET_BLOCK = BLOCKS.register("turret",
            () -> new TurretBlock(Block.Properties.of().strength(5.0F, 1200.0F).noOcclusion()));
    ...
    private static final ResourceKey<CreativeModeTab> CREATIVE = ResourceKey.create(Registries.CREATIVE_MODE_TAB, new ResourceLocation(ModMain.MOD_ID, "items"));

    @SubscribeEvent
    public static void creativeTab(RegisterEvent event){
        event.register(Registries.CREATIVE_MODE_TAB, tabs -> {
            tabs.register(CREATIVE, CreativeModeTab.builder()
                    .title(Component.translatable("item_group." + ModMain.MOD_ID + ".items"))
                    .icon(() -> new ItemStack(Items.EMERALD))
                    .displayItems((parameters, output) -> {
                        output.accept(LOOTED_GENERATOR_BLOCK.get());
                        output.accept(TERRAIN_GENERATOR_BLOCK.get());
                        output.accept(BASE_BLOCK.get());
